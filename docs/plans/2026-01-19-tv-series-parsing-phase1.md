...
**Goal:** Extend the release parser to handle TV series naming conventions from Sonarr, closing critical gaps for multi-episode releases, alternate formats, season packs, and daily shows.
...
**Files:**
- Modify: `pkg/release/release.go:143-166`
...
func TestInfo_Episodes_Slice(t *testing.T) {
	...
	if len(info.Episodes) != 3 {
		t.Errorf("Episodes length = %d, want 3", len(info.Episodes))
	}
	if info.Episodes[0] != 5 {
		t.Errorf("Episodes[0] = %d, want 5", info.Episodes[0])
	}
}
```
...
Modify `pkg/release/release.go` - change the Info struct:
...
type Info struct {
	...
	FullSeason      bool // Complete season release
	IsPartialSeason bool // Partial season (e.g., "Season 1 Part 2")
	SeasonPart      int  // Part number for partial seasons
...
Add Episodes []int slice for multi-episode releases, plus FullSeason,
IsPartialSeason, and SeasonPart fields for season pack detection.
...
**Files:**
- Modify: `pkg/release/parser.go:10-18`
- Test: `pkg/release/release_test.go`
...
Modify `pkg/release/parser.go` - add new regex patterns and update parsing:
...
var (
	...
	hdrRegex         = regexp.MustCompile(`(?i)\bHDR10\+|\b(HDR10Plus|HDR10|HDR|DV|DoVi|Dolby\.?Vision|HLG)\b`)
...
func TestParse_MultiEpisode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got.Episodes) != len(tt.wantEpisodes) {
				t.Errorf("Episodes length = %d, want %d", len(got.Episodes), len(tt.wantEpisodes))
			} else {
				for i, ep := range tt.wantEpisodes {
					if got.Episodes[i] != ep {
						t.Errorf("Episodes[%d] = %d, want %d", i, got.Episodes[i], ep)
					}
				}
			}
		})
...
Add new regex patterns and a helper function to `pkg/release/parser.go`:
...
// expandEpisodeRange creates a slice from start to end inclusive
...
func TestParse_SeasonPack(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.FullSeason != tt.wantFullSeason {
				t.Errorf("FullSeason = %v, want %v", got.FullSeason, tt.wantFullSeason)
			}
			if got.IsPartialSeason != tt.wantPartialSeason {
				t.Errorf("IsPartialSeason = %v, want %v", got.IsPartialSeason, tt.wantPartialSeason)
			}
			if got.SeasonPart != tt.wantSeasonPart {
				t.Errorf("SeasonPart = %v, want %v", got.SeasonPart, tt.wantSeasonPart)
			}
		})
	}
}
...
Expected: FAIL - FullSeason and IsPartialSeason will be false
...
var (
	...
	partialSeasonRegex = regexp.MustCompile(`(?i)(?:Season[\s.]?(\d{1,2})|S(\d{1,2}))[\s.]+(?:Part|Vol)[\s.]?(\d{1,2})`)
)
```
...
```go
	...
	if info.Episode == 0 && len(info.Episodes) == 0 {
		// Check for partial season first (more specific)
		if matches := partialSeasonRegex.FindStringSubmatch(normalized); len(matches) == 4 {
			season := matches[1]
			if season == "" {
				season = matches[2]
			}
...
git add pkg/release/parser.go pkg/release/release_test.go
git commit -m "$(cat <<'EOF'
feat(release): add season pack and partial season detection

Detect full season packs (Season 01, S01, Complete Season) and partial
seasons (Season 1 Part 2, S01 Vol 1). Sets FullSeason, IsPartialSeason,
and SeasonPart fields appropriately.
...
// monthNameToNumber converts month abbreviation to number string
...
func parseDailyDate(name string) string {
	...
	if matches := dailyWordMonthRegex.FindStringSubmatch(name); len(matches) == 4 {
		...
		month := monthNameToNumber(matches[2])
		year := matches[3]
	...
	if matches := dailyUSWordRegex.FindStringSubmatch(name); len(matches) == 4 {
		month := monthNameToNumber(matches[1])
		day := matches[2]
		if len(day) == 1 {
			day = "0" + day
		}
...
Update `parseAudio` function in `pkg/release/parser.go`:
...
func parseAudio(name string) AudioCodec {
	...
	// Match dd+ with optional space/dot and channel config
	if containsAny(lower, "ddp", "dd+", "eac3", "e-ac3") ||
		regexp.MustCompile(`(?i)\bdd\+[\s.]?\d`).MatchString(lower) {
		return AudioEAC3
	}

	// DD / AC3 - improved pattern matching
	// Match: dd5.1, dd2.0, dd7.1, dd 5.1, dd.5.1, ac3, dolby digital
	if containsAny(lower, "ac3", "dolby digital") ||
		regexp.MustCompile(`(?i)\bdd[\s.]?\d\.\d`).MatchString(lower) {
		return AudioAC3
	}
...
Run: `task test`
Expected: All tests PASS (some golden tests may now pass that previously didn't)
...
Add the following test cases to `goldenCases` in `pkg/release/golden_test.go`:

```go
	// === TV Series Multi-Episode (new cases) ===
	{
...
- Fixed audio patterns (DD.5.1, DD.2.0)
...
if info.FullSeason {
    fmt.Printf("  Full Season: yes\n")
}
if info.IsPartialSeason {
    fmt.Printf("  Partial Season: Part %d\n", info.SeasonPart)
}
```
...
Expected: Output shows Full Season: yes
...
detected, and FullSeason/IsPartialSeason flags for season packs.
...
3. **Season packs**: `Season 01`, `S01`, `Complete Season` detection
4. **Partial seasons**: `Season 1 Part 2` → `IsPartialSeason: true, SeasonPart: 2`
